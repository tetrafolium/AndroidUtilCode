...
public final class NetworkUtils {
  ...
  public static Utils.Task<Boolean>
  isAvailableAsync(@NonNull final Utils.Consumer<Boolean> consumer) {
   ...
   * <p>Must hold {@code <uses-permission
   * android:name="android.permission.INTERNET" />}</p> <p>The default ping ip:
   * 223.5.5.5</p>
   *
   * @param consumer The consumer.
   */
  @RequiresPermission(INTERNET)
  public static void
  isAvailableByPingAsync(final Utils.Consumer<Boolean> consumer) {
  ...
  @RequiresPermission(INTERNET)
  public static Utils.Task<Boolean>
  isAvailableByPingAsync(final String ip,
  ...
  public static boolean isAvailableByPing(final String ip) {
    ...
    ShellUtils.CommandResult result =
        ShellUtils.execCmd(String.format("ping -c 1 %s", realIp), false);
  ...
  @RequiresPermission(INTERNET)
  public static void
  isAvailableByDnsAsync(final Utils.Consumer<Boolean> consumer) {
   ...
   */
  @RequiresPermission(INTERNET)
  public static Utils.Task
  isAvailableByDnsAsync(final String domain,
                        @NonNull final Utils.Consumer<Boolean> consumer) {
    return UtilsBridge.doAsync(new Utils.Task<Boolean>(consumer) {
      @RequiresPermission(INTERNET)
      @Override
  ...
  public static boolean isAvailableByDns(final String domain) {
    final String realDomain =
        TextUtils.isEmpty(domain) ? "www.baidu.com" : domain;
  ...
  public static boolean getWifiEnabled() {
    ...
    WifiManager manager =
        (WifiManager)Utils.getApp().getSystemService(WIFI_SERVICE);
    if (manager == null)
      return false;
  ...
  @RequiresPermission(allOf = {ACCESS_WIFI_STATE, INTERNET})
  public static Utils.Task<Boolean>
  isWifiAvailableAsync(@NonNull final Utils.Consumer<Boolean> consumer) {
    return UtilsBridge.doAsync(new Utils.Task<Boolean>(consumer) {
      @RequiresPermission(allOf = {ACCESS_WIFI_STATE, INTERNET})
      @Override
      public Boolean doInBackground() {
   ...
   * @return the task
   */
  public static Utils.Task<String>
  getIPAddressAsync(final boolean useIPv4,
  ...
  @RequiresPermission(INTERNET)
  public static Utils.Task<String>
  getDomainAddressAsync(final String domain,
                        @NonNull final Utils.Consumer<String> consumer) {
    return UtilsBridge.doAsync(new Utils.Task<String>(consumer) {
      @RequiresPermission(INTERNET)
      @Override
  ...
  public static String getSSID() {
    WifiManager wm =
        (WifiManager)Utils.getApp().getApplicationContext().getSystemService(
            WIFI_SERVICE);
    if (wm == null)
      return "";
  ...
  public static void registerNetworkStatusChangedListener(
      final OnNetworkStatusChangedListener listener) {
   ...
   */
  public static boolean isRegisteredNetworkStatusChangedListener(
      final OnNetworkStatusChangedListener listener) {
   ...
   * @param listener The status of network changed listener.
   */
  public static void unregisterNetworkStatusChangedListener(
      final OnNetworkStatusChangedListener listener) {
  ...
  public static final class NetworkChangedReceiver extends BroadcastReceiver {
        ...
        public void run() {
          ...
          if (preSize == 0 && mListeners.size() == 1) {
            ...
            Utils.getApp().registerReceiver(
                NetworkChangedReceiver.getInstance(), intentFilter);
          }
        }
      });
    }
      ...
      UtilsBridge.runOnUiThread(new Runnable() {
        ...
        public void run() {
          ...
          if (preSize == 1 && mListeners.size() == 0) {
            Utils.getApp().unregisterReceiver(
    ...
    private static class LazyHolder {
      private static final NetworkChangedReceiver INSTANCE =
  ...
  //        Utils.getApp().getSystemService(Context.CONNECTIVITY_SERVICE); if
  //        (cm == null) return; NetworkCallbackImpl networkCallback =
  //        NetworkCallbackImpl.LazyHolder.INSTANCE; NetworkRequest.Builder
  //        builder = new NetworkRequest.Builder(); NetworkRequest request =
  //        builder.build(); cm.registerNetworkCallback(new
  //        NetworkRequest.Builder().build(), networkCallback);
  //    }
  //
  //
  //    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
  //    public static final class NetworkCallbackImpl extends
  ...
  //                }
  //                LogUtils.d(TAG, "onCapabilitiesChanged: " + network + ", " +
  //                cap);
  //            }
  //        }
  //
  ...
  //            LogUtils.d(TAG, "onLinkPropertiesChanged: " + network + ", " +
  //            lp);
  //        }
  //
  //        @Override
  //        public void onBlockedStatusChanged(@NonNull Network network, boolean
  //        blocked) {
  //            super.onBlockedStatusChanged(network, blocked);
  //            LogUtils.d(TAG, "onBlockedStatusChanged: " + network + ", " +
  //            blocked);
  //        }
  //
  //        private static class LazyHolder {
  //            private static final NetworkCallbackImpl INSTANCE = new
  //            NetworkCallbackImpl();
  //        }
  //    }

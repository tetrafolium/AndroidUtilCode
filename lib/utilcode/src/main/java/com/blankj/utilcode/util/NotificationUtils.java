...
public class NotificationUtils {
   ...
   */
  public static void
  notify(int id, Utils.Consumer<NotificationCompat.Builder> consumer) {
   ...
   */
  public static void
  notify(int id, ChannelConfig channelConfig,
         Utils.Consumer<NotificationCompat.Builder> consumer) {
   ...
   * Post a notification to be shown in the status bar.
   *
   * @param tag           A string identifier for this notification.  May be
   *     {@code null}.
   * @param id            An identifier for this notification.
   * @param channelConfig The notification channel of config.
   * @param consumer      The consumer of create the builder of notification.
   */
  public static void
  notify(String tag, int id, ChannelConfig channelConfig,
         Utils.Consumer<NotificationCompat.Builder> consumer) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      NotificationManager nm =
          (NotificationManager)Utils.getApp().getSystemService(
    ...
    NotificationCompat.Builder builder =
        new NotificationCompat.Builder(Utils.getApp());
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
   ...
   * android:name="android.permission.EXPAND_STATUS_BAR" />}</p>
   *
  ...
  public static void setNotificationBarVisibility(final boolean isVisible) {
    ...
    if (isVisible) {
      methodName =
          (Build.VERSION.SDK_INT <= 16) ? "expand" : "expandNotificationsPanel";
    } else {
      methodName =
  ...
  public static class ChannelConfig {
    ...
    public static final ChannelConfig DEFAULT_CHANNEL_CONFIG =
        new ChannelConfig(Utils.getApp().getPackageName(),
                          Utils.getApp().getPackageName(), IMPORTANCE_DEFAULT);

    private NotificationChannel mNotificationChannel;

    public ChannelConfig(String id, CharSequence name,
    ...
    /**
     * Sets whether or not notifications posted to this channel can interrupt
     * the user in
     * {@link
     * android.app.NotificationManager.Policy#INTERRUPTION_FILTER_PRIORITY}
     * mode. <p> Only modifiable by the system and notification ranker.
     */
     ...
     * channel, if lights are
     * {@link NotificationChannel#enableLights(boolean) enabled} on this channel
     * and the device supports that feature. <p> Only modifiable before the
     * channel is submitted to
     * {@link
     * NotificationManager#createNotificationChannel(NotificationChannel)}.
     */
    ...
    /**
     * Sets whether notifications posted to this channel appear on the
     * lockscreen or not, and if so, whether they appear in a redacted form. See
     * e.g. {@link Notification#VISIBILITY_SECRET}. <p> Only modifiable by the
     * system and notification ranker.
     */
     ...
     *
     * <p>The recommended maximum length is 40 characters; the value may be
     * truncated if it is too long.
     */
    ...
    /**
     * Sets whether notifications posted to this channel can appear as
     * application icon badges in a Launcher. <p> Only modifiable before the
     * channel is submitted to
     * {@link
     * NotificationManager#createNotificationChannel(NotificationChannel)}.
     *
     ...
     * channel and its audio attributes. Notification channels with an {@link
     * NotificationChannel#getImportance() importance} of at least {@link
     * NotificationManager#IMPORTANCE_DEFAULT} should have a sound. <p> Only
    ...
    /**
     * Sets the vibration pattern for notifications posted to this channel. If
     * the provided pattern is valid (non-null, non-empty), will {@link
     * NotificationChannel#enableVibration(boolean)} enable vibration} as well.
     * Otherwise, vibration will be disabled. <p> Only modifiable before the
     * channel is submitted to
     * {@link
     * NotificationManager#createNotificationChannel(NotificationChannel)}.
     */

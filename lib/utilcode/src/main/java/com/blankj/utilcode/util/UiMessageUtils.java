...
public final class UiMessageUtils implements Handler.Callback {
  ...
  public static UiMessageUtils getInstance() { return LazyHolder.INSTANCE; }
   ...
   */
  public final void send(final int id) { mHandler.sendEmptyMessage(id); }
  ...
  public void removeListener(@NonNull final UiMessageCallback listener) {
    synchronized (mListenersUniversal) {
      if (DEBUG && !mListenersUniversal.contains(listener)) {
        Log.w(TAG, "Trying to remove a listener that is not registered. " +
                       listener.toString());
      }
  ...
  public void removeListeners(final int id) {
    if (DEBUG) {
      ...
      if (callbacks == null || callbacks.size() == 0) {
        Log.w(
            TAG,
            "Trying to remove specific listeners that are not registered. ID " +
                id);
      }
    }
  ...
  public void removeListener(final int id,
                             @NonNull final UiMessageCallback listener) {
    synchronized (mListenersSpecific) {
      ...
      if (callbacks != null && !callbacks.isEmpty()) {
        if ((DEBUG) && (!callbacks.contains(listener))) {
          Log.w(
              TAG,
              "Trying to remove specific listener that is not registered. ID " +
                  id + ", " + listener);
          return;
        }
  ...
  public boolean handleMessage(Message msg) {
    ...
    synchronized (mListenersSpecific) {
      final List<UiMessageCallback> idListeners =
          mListenersSpecific.get(msg.what);
      if (idListeners != null) {
  ...
  private void logMessageHandling(@NonNull final UiMessage msg) {
    final List<UiMessageCallback> idListeners =
        mListenersSpecific.get(msg.getId());

    if ((idListeners == null || idListeners.size() == 0) &&
        mListenersUniversal.size() == 0) {
      Log.w(TAG, "Delivering FAILED for message ID " + msg.getId() +
                     ". No listeners. " + msg.toString());
    } else {
      ...
      synchronized (mListenersUniversal) {
        if (mListenersUniversal.size() == 0) {
          ...
          for (int i = 0; i < mListenersUniversal.size(); i++) {
            stringBuilder.append(
                mListenersUniversal.get(i).getClass().getSimpleName());
  ...
  public static final class UiMessage {
    ...
    private UiMessage(final Message message) { this.mMessage = message; }

    private void setMessage(final Message message) { mMessage = message; }

    public int getId() { return mMessage.what; }

    public Object getObject() { return mMessage.obj; }

    @Override
    public String toString() {
      return "{ "
          + "id=" + getId() + ", obj=" + getObject() + " }";
    }
  }

...
public final class SpanUtils {
   ...
   * <p>Must set {@code
   * view.setMovementMethod(LinkMovementMethod.getInstance())}</p>
   *
   ...
   * @return the single {@link SpanUtils} instance
   */
  public SpanUtils setBlur(@FloatRange(from = 0, fromInclusive = false)
                           final float radius, final Blur style) {
   ...
   * @return the single {@link SpanUtils} instance
   */
  public SpanUtils setShadow(@FloatRange(from = 0, fromInclusive = false)
                             final float radius, final float dx, final float dy,
  ...
  public SpannableStringBuilder get() { return mBuilder; }
  ...
  private void updateCharCharSequence() {
    ...
    if (start == 0 &&
        lineHeight != -1) { // bug of LineHeightSpan when first line
    ...
    if (foregroundColor != COLOR_DEFAULT) {
      mBuilder.setSpan(new ForegroundColorSpan(foregroundColor), start, end,
                       flag);
    }
    if (backgroundColor != COLOR_DEFAULT) {
      mBuilder.setSpan(new BackgroundColorSpan(backgroundColor), start, end,
                       flag);
    }
    if (first != -1) {
      mBuilder.setSpan(new LeadingMarginSpan.Standard(first, rest), start, end,
                       flag);
    }
    if (quoteColor != COLOR_DEFAULT) {
      mBuilder.setSpan(
          new CustomQuoteSpan(quoteColor, stripeWidth, quoteGapWidth), start,
          end, flag);
    }
    ...
    if (fontSize != -1) {
      mBuilder.setSpan(new AbsoluteSizeSpan(fontSize, fontSizeIsDp), start, end,
                       flag);
    }
    if (proportion != -1) {
    ...
    if (lineHeight != -1) {
      mBuilder.setSpan(new CustomLineHeightSpan(lineHeight, alignLine), start,
                       end, flag);
    }
    if (isStrikethrough) {
    ...
    if (blurRadius != -1) {
      mBuilder.setSpan(
          new MaskFilterSpan(new BlurMaskFilter(blurRadius, style)), start, end,
          flag);
    }
    if (shader != null) {
    ...
    if (shadowRadius != -1) {
      mBuilder.setSpan(
          new ShadowSpan(shadowRadius, shadowDx, shadowDy, shadowColor), start,
          end, flag);
    }
    if (spans != null) {
  ...
  private void updateImage() {
    ...
    int end = mBuilder.length();
    if (imageBitmap != null) {
      mBuilder.setSpan(new CustomImageSpan(imageBitmap, alignImage), start, end,
                       flag);
    } else if (imageDrawable != null) {
      mBuilder.setSpan(new CustomImageSpan(imageDrawable, alignImage), start,
                       end, flag);
    } else if (imageUri != null) {
      mBuilder.setSpan(new CustomImageSpan(imageUri, alignImage), start, end,
                       flag);
    } else if (imageResourceId != -1) {
      mBuilder.setSpan(new CustomImageSpan(imageResourceId, alignImage), start,
                       end, flag);
    }
  }
  ...
  static class VerticalAlignSpan extends ReplacementSpan {
    ...
    public void draw(@NonNull Canvas canvas, CharSequence text, int start,
      ...
      //            int need = height - (v + fm.descent - fm.ascent -
      //            spanstartv); if (need > 0) {
  ...
  static class SpaceSpan extends ReplacementSpan {
    ...
    private SpaceSpan(final int width) { this(width, Color.TRANSPARENT); }
    ...
    public void draw(@NonNull final Canvas canvas, final CharSequence text,
                     @IntRange(from = 0) final int start,
                     @IntRange(from = 0) final int end, final float x,
  ...
  static class CustomQuoteSpan implements LeadingMarginSpan {
    ...
    private CustomQuoteSpan(final int color, final int stripeWidth,
                            final int gapWidth) {
      super();
      this.color = color;
      this.stripeWidth = stripeWidth;
  ...
  static class CustomBulletSpan implements LeadingMarginSpan {
    ...
    private CustomBulletSpan(final int color, final int radius,
                             final int gapWidth) {
      this.color = color;
      this.radius = radius;
      this.gapWidth = gapWidth;
      ...
      if (((Spanned)text).getSpanStart(this) == start) {
        ...
        if (c.isHardwareAccelerated()) {
          if (sBulletPath == null) {
            ...
            // Bullet is slightly better to avoid aliasing artifacts on mdpi
            // devices.
  ...
  static class CustomImageSpan extends CustomDynamicDrawableSpan {
    ...
    private CustomImageSpan(@DrawableRes final int resourceId,
  ...
  static abstract class CustomDynamicDrawableSpan extends ReplacementSpan {
    ...
    private CustomDynamicDrawableSpan() { mVerticalAlignment = ALIGN_BOTTOM; }
  ...
  static class ShaderSpan extends CharacterStyle implements UpdateAppearance {
    ...
    private ShaderSpan(final Shader shader) { this.mShader = shader; }
  ...
  static class ShadowSpan extends CharacterStyle implements UpdateAppearance {
    ...
    private ShadowSpan(final float radius, final float dx, final float dy,
                       final int shadowColor) {
      this.radius = radius;
      this.dx = dx;
      this.dy = dy;
  ...
  private static class SerializableSpannableStringBuilder
      extends SpannableStringBuilder implements Serializable {

...
public final class BrightnessUtils {
  ...
  public static boolean isAutoBrightnessEnabled() {
    try {
      int mode = Settings.System.getInt(Utils.getApp().getContentResolver(),
   ...
   * android:name="android.permission.WRITE_SETTINGS" />}</p>
   *
   ...
   * android:name="android.permission.WRITE_SETTINGS" />}</p> 并得到授权
   *
   * @param brightness 亮度值
   */
  public static boolean
  setBrightness(@IntRange(from = 0, to = 255) final int brightness) {
    ContentResolver resolver = Utils.getApp().getContentResolver();
    boolean b = Settings.System.putInt(

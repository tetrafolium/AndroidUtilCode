...
final class UtilsActivityLifecycleImpl
  ...
  static final UtilsActivityLifecycleImpl INSTANCE =
      new UtilsActivityLifecycleImpl();

  private final LinkedList<Activity> mActivityList = new LinkedList<>();

  private final List<Utils.OnAppStatusChangedListener> mStatusListeners =
      new ArrayList<>();
  private final Map<Activity, List<Utils.ActivityLifecycleCallbacks>>
  ...
  void init(Application app) { app.registerActivityLifecycleCallbacks(this); }
  ...
  void addActivityLifecycleCallbacks(
  ...
  Application getApplicationByReflect() {
    try {
      ...
      Object app =
          activityThreadClass.getMethod("getApplication").invoke(thread);
      if (app == null) {
        return null;
      }
  ...
  private void addActivityLifecycleCallbacksInner(
      final Activity activity,
      final Utils.ActivityLifecycleCallbacks lifecycleCallbacks) {
    List<Utils.ActivityLifecycleCallbacks> callbacks =
        mActivityLifecycleCallbacksMap.get(activity);
    if (callbacks == null) {
  ...
  void removeActivityLifecycleCallbacks(
      final Activity activity,
  ...
  private void removeActivityLifecycleCallbacksInner(
      final Activity activity,
      final Utils.ActivityLifecycleCallbacks lifecycleCallbacks) {
    List<Utils.ActivityLifecycleCallbacks> callbacks =
        mActivityLifecycleCallbacksMap.get(activity);
  ...
  private void consumeActivityLifecycleCallbacks(Activity activity,
                                                 Lifecycle.Event event) {
    List<Utils.ActivityLifecycleCallbacks> listeners =
        mActivityLifecycleCallbacksMap.get(activity);
    if (listeners != null) {
  ...
  private List<Activity> getActivitiesByReflect() {
    ...
    try {
      ...
      Field mActivitiesField =
          activityThread.getClass().getDeclaredField("mActivities");
      mActivitiesField.setAccessible(true);
      Object mActivities = mActivitiesField.get(activityThread);
      if (!(mActivities instanceof Map)) {
        return list;
      }
      Map<Object, Object> binder_activityClientRecord_map =
  ...
  private Object getActivityThreadInActivityThreadStaticMethod() {
    try {
      Class activityThreadClass = Class.forName("android.app.ActivityThread");
      return activityThreadClass.getMethod("currentActivityThread")
          .invoke(null);
    } catch (Exception e) {
      Log.e("UtilsActivityLifecycle",
  ...
  private Object getActivityThreadInLoadedApkField() {
    try {
      ...
      Field mActivityThreadField =
          mLoadedApk.getClass().getDeclaredField("mActivityThread");
      mActivityThreadField.setAccessible(true);
      return mActivityThreadField.get(mLoadedApk);
    } catch (Exception e) {
      Log.e("UtilsActivityLifecycle",
            "getActivityThreadInLoadedApkField: " + e.getMessage());
      return null;
    }
  }

...
public final class BusUtils {
  ...
  private void registerBus(String tag, String className, String funName,
                           String paramType, String paramName, boolean sticky,
                           String threadMode) {
    registerBus(tag, className, funName, paramType, paramName, sticky,
                threadMode, 0);
  }

  private void registerBus(String tag, String className, String funName,
                           String paramType, String paramName, boolean sticky,
                           String threadMode, int priority) {
    List<BusInfo> busInfoList = mTag_BusInfoListMap.get(tag);
    if (busInfoList == null) {
      busInfoList = new ArrayList<>();
      mTag_BusInfoListMap.put(tag, busInfoList);
    }
    busInfoList.add(new BusInfo(className, funName, paramType, paramName,
  ...
  public static void post(final String tag) { post(tag, NULL); }
  ...
  public static String toString_() { return getInstance().toString(); }
  ...
  private static BusUtils getInstance() { return LazyHolder.INSTANCE; }
    ...
    if (tags == null) {
      synchronized (mClassName_TagsMap) {
        ...
        if (tags == null) {
          ...
          for (Map.Entry<String, List<BusInfo>> entry :
               mTag_BusInfoListMap.entrySet()) {
  ...
  private void processSticky(final Object bus) {
    Map<String, Object> tagArgMap =
        mClassName_Tag_Arg4StickyMap.get(bus.getClass().getName());
    if (tagArgMap == null)
      return;
  ...
  private Method getMethodByBusInfo(BusInfo busInfo) {
    try {
      if ("".equals(busInfo.paramType)) {
        return Class.forName(busInfo.className)
            .getDeclaredMethod(busInfo.funName);
      } else {
        return Class.forName(busInfo.className)
            .getDeclaredMethod(busInfo.funName,
                               getClassName(busInfo.paramType));
      }
    } catch (ClassNotFoundException e) {
      e.printStackTrace();
  ...
  private void invokeMethod(final String tag, final Object arg,
                            final BusInfo busInfo, final boolean sticky) {
  ...
  private void realInvokeMethod(final String tag, Object arg, BusInfo busInfo,
                                boolean sticky) {
  ...
  private void postStickyInner(final String tag, final Object arg) {
    ...
    for (BusInfo busInfo : busInfoList) {
      ...
      synchronized (mClassName_Tag_Arg4StickyMap) {
        Map<String, Object> tagArgMap =
            mClassName_Tag_Arg4StickyMap.get(busInfo.className);
        if (tagArgMap == null) {
  ...
  static void registerBus4Test(String tag, String className, String funName,
                               String paramType, String paramName,
                               boolean sticky, String threadMode,
                               int priority) {
    getInstance().registerBus(tag, className, funName, paramType, paramName,
                              sticky, threadMode, priority);
  }
  ...
  private static final class BusInfo {
    ...
    BusInfo(String className, String funName, String paramType,
    ...
    public String toString() {
      return "BusInfo { desc: " + className + "#" + funName +
          ("".equals(paramType) ? "()"
                                : ("(" + paramType + " " + paramName + ")")) +
          ", sticky: " + sticky + ", threadMode: " + threadMode +
          ", method: " + method + ", priority: " + priority + " }";
    }
  }

  public enum ThreadMode { MAIN, IO, CPU, CACHED, SINGLE, POSTING }

...
public void writeFileFromIS() throws Exception {
	...
	FileIOUtils.writeFileFromIS(
		PATH_TEMP + "writeFileFromIS.txt", is,
		new FileIOUtils.OnProgressUpdateListener() {
			@Override
...
public void writeFileFromBytesByStream() throws Exception {
	...
	FileIOUtils.writeFileFromBytesByStream(
		PATH_TEMP + "writeFileFromBytesByStream.txt", bytes,
		new FileIOUtils.OnProgressUpdateListener() {
			@Override
			public void onProgressUpdate(double progress) {
			        System.out.println(String.format("%.2f", progress));
...
public void writeFileFromString() {
	...
	FileIOUtils.writeFileFromString(PATH_TEMP + "writeFileFromString.txt",
	                                sb.toString());
}
...
public void readFile2List() {
	writeFileFromString();
	for (String s :
	     FileIOUtils.readFile2List(PATH_TEMP + "writeFileFromString.txt")) {
...
public void readFile2String() {
	writeFileFromString();
	System.out.println(
		FileIOUtils.readFile2String(PATH_TEMP + "writeFileFromString.txt"));
}

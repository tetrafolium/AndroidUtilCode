...
public class SwipeRightMenu extends LinearLayout {
  ...
  private static final AccelerateInterpolator OPEN_INTERPOLATOR =
      new AccelerateInterpolator(0.5f);
  private static final DecelerateInterpolator CLOSE_INTERPOLATOR =
  ...
  public SwipeRightMenu(Context context) { this(context, null); }
  ...
  public boolean isOpen() { return swipeMenuOpened != null; }
  ...
  public boolean dispatchTouchEvent(MotionEvent event) {
    ...
    case MotionEvent.ACTION_MOVE:
      if (mState == STATE_DOWN && Math.abs(x - mDownX) < MIN_DISTANCE_MOVE &&
      ...
      scrollTo(Math.max(Math.min(getScrollX() - x + mLastX, mMenusWidth), 0),
               0);
    ...
    case MotionEvent.ACTION_CANCEL:
      try {
        ...
        if (swipeMenuOpened != null &&
            swipeMenuOpened.get() == this) { // 如果之前是展开状态
          if (getScrollX() <
              mMenusWidth - THRESHOLD_DISTANCE) { // 超过阈值则关闭
            close(true);
          } else { // 否则还是打开
            open(true);
          }
  ...
  private void updateLeftMarginByPercent(float percent) {
    for (MenuBean menu : mMenus) {
      menu.getParams().leftMargin =
          (int)(menu.getCloseMargin() +
                percent * (menu.getOpenMargin() - menu.getCloseMargin()));
  ...
  private void close(boolean isAnim) {
    ...
    if (isAnim) {
      ...
      for (final MenuBean menu : mMenus) {
        ValueAnimator menuAnim = ValueAnimator.ofInt(
  ...
  private static class MenuBean {
    ...
    public LayoutParams getParams() { return mParams; }

    public int getWidth() { return mWidth; }

    public int getCloseMargin() { return mCloseMargin; }

    public int getOpenMargin() { return mOpenMargin; }

    public void setCloseMargin(int closeMargin) { mCloseMargin = closeMargin; }

    public void setOpenMargin(int openMargin) { mOpenMargin = openMargin; }
  }
}

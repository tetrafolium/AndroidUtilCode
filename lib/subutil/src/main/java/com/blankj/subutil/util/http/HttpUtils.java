...
public final class HttpUtils {
  ...
  private HttpUtils(@NonNull Config config) { mConfig = config; }

  public static HttpUtils getInstance(@NonNull Config config) {
    if (sHttpUtils == null) {
      synchronized (HttpUtils.class) { sHttpUtils = new HttpUtils(config); }
    }
    return sHttpUtils;
  }

  public static void call(@NonNull final Request request,
                          @NonNull final ResponseCallback callback) {
    new Call(request, callback).run();
  }

  private static HttpURLConnection getConnection(final Request request)
      throws IOException {
  ...
  private static void addBody(HttpURLConnection conn, Request.Body body)
      throws IOException {
    if (body == null) {
      ...
      BufferedOutputStream bos =
          new BufferedOutputStream(conn.getOutputStream(), 10240);
      if (body.bis != null) {
  ...
  private static void addHeader(final HttpURLConnection conn,
    ...
    public void run() {
      ...
      try {
        ...
        if (responseCode == 200) {
          ...
          callback.onFailed(new NetworkErrorException(

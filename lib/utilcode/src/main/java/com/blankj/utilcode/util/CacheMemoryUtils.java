...
public final class CacheMemoryUtils implements CacheConstants {
   ...
   * @return the single {@link CacheMemoryUtils} instance
   */
  public static CacheMemoryUtils getInstance(final String cacheKey,
                                             final int maxCount) {
    ...
    if (cache == null) {
      synchronized (CacheMemoryUtils.class) {
        ...
        if (cache == null) {
          cache = new CacheMemoryUtils(
              cacheKey, new LruCache<String, CacheValue>(maxCount));
  ...
  private CacheMemoryUtils(String cacheKey,
                           LruCache<String, CacheValue> memoryCache) {
    mCacheKey = cacheKey;
  ...
  public void put(@NonNull final String key, final Object value, int saveTime) {
    if (value == null)
      return;
    long dueTime =
        saveTime < 0 ? -1 : System.currentTimeMillis() + saveTime * 1000;
   ...
   * @return the count of cache
   */
  public int getCacheCount() { return mMemoryCache.size(); }

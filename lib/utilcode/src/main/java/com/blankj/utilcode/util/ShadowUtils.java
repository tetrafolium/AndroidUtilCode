...
public class ShadowUtils {
  ...
  public static class Config {
    ...
    public Config setShadowSize(int size) { return setShadowSize(size, size); }
    ...
    Drawable apply(Drawable src) {
      ...
      drawable.addState(new int[] {android.R.attr.state_pressed},
                        new ShadowDrawable(src, getShadowRadius(),
                                           getShadowSizeNormal(),
                                           getShadowMaxSizeNormal(),
                                           mShadowColorPressed, isCircle));
      drawable.addState(StateSet.WILD_CARD,
                        new ShadowDrawable(src, getShadowRadius(),
                                           getShadowSizePressed(),
                                           getShadowMaxSizePressed(),
                                           mShadowColorNormal, isCircle));
      return drawable;
    }
  ...
  public static class ShadowDrawable extends DrawableWrapper {
    ...
    public ShadowDrawable(Drawable content, float radius, float shadowSize,
                          float maxShadowSize, int shadowColor,
                          boolean isCircle) {
      super(content);
    ...
    public boolean getPadding(Rect padding) {
      int vOffset = (int)Math.ceil(calculateVerticalPadding(
          mRawMaxShadowSize, mCornerRadius, mAddPaddingForCorners));
      int hOffset = (int)Math.ceil(calculateHorizontalPadding(
          mRawMaxShadowSize, mCornerRadius, mAddPaddingForCorners));
      padding.set(hOffset, vOffset, hOffset, vOffset);
      return true;
    }

    private float calculateVerticalPadding(float maxShadowSize,
                                           float cornerRadius,
                                           boolean addPaddingForCorners) {
      if (addPaddingForCorners) {
        return (float)(maxShadowSize * mShadowMultiplier +
                       (1 - COS_45) * cornerRadius);
      } else {
    ...
    calculateHorizontalPadding(float maxShadowSize, float cornerRadius,
                               boolean addPaddingForCorners) {
      if (addPaddingForCorners) {
    ...
    private void drawShadow(Canvas canvas) {
      ...
      canvas.rotate(mRotation, mContentBounds.centerX(),
                    mContentBounds.centerY());

      final float edgeShadowTop = -mCornerRadius - mShadowSize;
      final float shadowOffset = mCornerRadius;
      final boolean drawHorizontalEdges =
          mContentBounds.width() - 2 * shadowOffset > 0;
      final boolean drawVerticalEdges =
          mContentBounds.height() - 2 * shadowOffset > 0;

      final float shadowOffsetTop =
          mRawShadowSize - (mRawShadowSize * mShadowTopScale);
      final float shadowOffsetHorizontal =
          mRawShadowSize - (mRawShadowSize * mShadowHorizScale);
      final float shadowOffsetBottom =
          mRawShadowSize - (mRawShadowSize * mShadowBottomScale);

      final float shadowScaleHorizontal =
          shadowOffset == 0
              ? 1
              : shadowOffset / (shadowOffset + shadowOffsetHorizontal);
      final float shadowScaleTop =
          shadowOffset == 0 ? 1
                            : shadowOffset / (shadowOffset + shadowOffsetTop);
      final float shadowScaleBottom =
          shadowOffset == 0
              ? 1
              : shadowOffset / (shadowOffset + shadowOffsetBottom);

      // LT
      int saved = canvas.save();
      canvas.translate(mContentBounds.left + shadowOffset,
                       mContentBounds.top + shadowOffset);
    ...
    private void buildShadowCorners() {
      if (isCircle) {
        ...
        if (shadowRadius > 0f) {
          ...
          mCornerShadowPaint.setShader(new RadialGradient(
              0, 0, shadowRadius,
      ...
      RectF innerBounds = new RectF(-mCornerRadius, -mCornerRadius,
      ...
      mEdgeShadowPaint.setShader(new LinearGradient(
          0, innerBounds.top, 0, outerBounds.top, mShadowStartColor,
          mShadowEndColor, Shader.TileMode.CLAMP));
      mEdgeShadowPaint.setAntiAlias(false);
    }

    private void buildComponents(Rect bounds) {
      // Card is offset mShadowMultiplier * maxShadowSize to account for the
      // shadow shift. We could have different top-bottom offsets to avoid extra
      // gap above but in that case center aligning Views inside the CardView
      ...
      getWrappedDrawable().setBounds(
          (int)mContentBounds.left, (int)mContentBounds.top,
    ...
    public float getShadowSize() { return mRawShadowSize; }

    public float getMaxShadowSize() { return mRawMaxShadowSize; }

    public float getMinWidth() {
      final float content = 2 * Math.max(mRawMaxShadowSize,
                                         mCornerRadius + mRawMaxShadowSize / 2);
    ...
    public float getMinHeight() {
      final float content =
          2 *
          Math.max(mRawMaxShadowSize,
                   mCornerRadius + mRawMaxShadowSize * mShadowMultiplier / 2);
  ...
  static class DrawableWrapper extends Drawable implements Drawable.Callback {
    ...
    public void draw(Canvas canvas) { this.mDrawable.draw(canvas); }
    ...
    public void setDither(boolean dither) { this.mDrawable.setDither(dither); }

    public void setFilterBitmap(boolean filter) {
      this.mDrawable.setFilterBitmap(filter);
    }

    public void setAlpha(int alpha) { this.mDrawable.setAlpha(alpha); }
    ...
    public void invalidateDrawable(Drawable who) { this.invalidateSelf(); }

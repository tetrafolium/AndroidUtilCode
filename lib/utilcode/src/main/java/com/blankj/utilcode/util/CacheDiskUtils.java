...
public final class CacheDiskUtils implements CacheConstants {
   ...
   * @return the single {@link CacheDiskUtils} instance
   */
  public static CacheDiskUtils getInstance(final long maxSize,
                                           final int maxCount) {
  ...
  public static CacheDiskUtils getInstance(@NonNull final File cacheDir,
    ...
    final String cacheKey =
        cacheDir.getAbsoluteFile() + "_" + maxSize + "_" + maxCount;
  ...
  private CacheDiskUtils(final String cacheKey, final File cacheDir,
                         final long maxSize, final int maxCount) {
    mCacheKey = cacheKey;
    mCacheDir = cacheDir;
    mMaxSize = maxSize;
    mMaxCount = maxCount;
  ...
  private DiskCacheManager getDiskCacheManager() {
    if (mCacheDir.exists()) {
      if (mDiskCacheManager == null) {
        mDiskCacheManager =
            new DiskCacheManager(mCacheDir, mMaxSize, mMaxCount);
      }
    } else {
      if (mCacheDir.mkdirs()) {
        mDiskCacheManager =
        ...
        Log.e("CacheDiskUtils",
              "can't make dirs in " + mCacheDir.getAbsolutePath());
      }
    }
  ...
  public void put(@NonNull final String key, final JSONObject value,
                  final int saveTime) {
    realPutBytes(TYPE_JSON_OBJECT + key, UtilsBridge.jsonObject2Bytes(value),
                 saveTime);
  }
  ...
  public void put(@NonNull final String key, final JSONArray value,
                  final int saveTime) {
    realPutBytes(TYPE_JSON_ARRAY + key, UtilsBridge.jsonArray2Bytes(value),
                 saveTime);
  }
  ...
  public void put(@NonNull final String key, final Drawable value,
                  final int saveTime) {
    realPutBytes(TYPE_DRAWABLE + key, UtilsBridge.drawable2Bytes(value),
                 saveTime);
  }
  ...
  public void put(@NonNull final String key, final Parcelable value,
                  final int saveTime) {
    realPutBytes(TYPE_PARCELABLE + key, UtilsBridge.parcelable2Bytes(value),
                 saveTime);
  }
  ...
  public void put(@NonNull final String key, final Serializable value,
                  final int saveTime) {
    realPutBytes(TYPE_SERIALIZABLE + key, UtilsBridge.serializable2Bytes(value),
                 saveTime);
  }
  ...
  private static final class DiskCacheManager {
    ...
    private DiskCacheManager(final File cacheDir, final long sizeLimit,
  ...
  private static final class DiskCacheHelper {
    ...
    private static byte[] newByteArrayWithTime(final int second,
                                               final byte[] data) {
    ...
    private static boolean hasTimeInfo(final byte[] data) {
      return data != null && data.length >= TIME_INFO_LEN && data[0] == '_' &&
          data[1] == '$' && data[12] == '$' && data[13] == '_';
    }
  }
}

...
public static boolean isDeviceRooted() {
	...
	String[] locations = {
		...
		"/data/local/bin/", "/data/local/",          "/system/sbin/",
		"/usr/bin/",        "/vendor/bin/"
	};
...
public static boolean isAdbEnabled() {
	return Settings.Secure.getInt(Utils.getApp().getContentResolver(),
...
public static String getAndroidID() {
	String id = Settings.Secure.getString(Utils.getApp().getContentResolver(),
...
@RequiresPermission(allOf = {
		ACCESS_WIFI_STATE, INTERNET, CHANGE_WIFI_STATE
	})
...
private static boolean isAddressNotInExcepts(final String address,
...
private static String getMacAddressByFile() {
	ShellUtils.CommandResult result =
		UtilsBridge.execCmd("getprop wifi.interface", false);
	if (result.result == 0) {
		String name = result.successMsg;
		if (name != null) {
			result = UtilsBridge.execCmd("cat /sys/class/net/" + name + "/address",
			                             false);
			if (result.result == 0) {
...
/**
 * Return an ordered list of ABIs supported by this device. The most preferred
...
public static String getUniqueDeviceId(String prefix, boolean useCache) {
	...
	if (udid == null) {
		synchronized (DeviceUtils.class) {
			if (udid == null) {
				final String id =
					UtilsBridge.getSpUtils4Utils().getString(KEY_UDID, null);
				if (id != null) {
					udid = id;
					return udid;
				}
...
private static String getUdid(String prefix, String id) {
	...
	return prefix +
	       UUID.nameUUIDFromBytes(id.getBytes()).toString().replace("-", "");
}
}

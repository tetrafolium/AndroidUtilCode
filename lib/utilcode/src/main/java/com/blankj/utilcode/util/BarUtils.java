...
private static void
subtractMarginTopEqualStatusBarHeight(final Window window) {
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT)
		return;
	View withTag = window.getDecorView().findViewWithTag(TAG_OFFSET);
	if (withTag == null)
...
public static void setStatusBarColor(@NonNull final View fakeStatusBar,
	...
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT)
		return;
	Activity activity =
		UtilsBridge.getActivityByContext(fakeStatusBar.getContext());
	if (activity == null)
		return;
	transparentStatusBar(activity);
 ...
 */
public static void
setStatusBarColor4Drawer(@NonNull final DrawerLayout drawer,
 ...
 * @param fakeStatusBar The fake status bar view.
 * @param color         The status bar's color.
 * @param isTop         True to set DrawerLayout at the top layer, false
 *     otherwise.
 */
public static void
setStatusBarColor4Drawer(@NonNull final DrawerLayout drawer,
                         @NonNull final View fakeStatusBar,
                         @ColorInt final int color, final boolean isTop) {
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT)
		return;
	Activity activity =
		UtilsBridge.getActivityByContext(fakeStatusBar.getContext());
...
private static View applyStatusBarColor(final Activity activity,
                                        final int color, boolean isDecor) {
	return applyStatusBarColor(activity.getWindow(), color, isDecor);
}

private static View applyStatusBarColor(final Window window, final int color,
                                        boolean isDecor) {
	ViewGroup parent =
		isDecor ? (ViewGroup)window.getDecorView()
...
public static int getActionBarHeight() {
	...
	if (Utils.getApp().getTheme().resolveAttribute(android.R.attr.actionBarSize,
	                                               tv, true)) {
 ...
 */
public static void setNavBarVisibility(@NonNull final Activity activity,
                                       boolean isVisible) {
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT)
		return;
 ...
 * @param isVisible True to set navigation bar visible, false otherwise.
 */
public static void setNavBarVisibility(@NonNull final Window window,
                                       boolean isVisible) {
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT)
		return;
...
public static void setNavBarColor(@NonNull final Activity activity,
                                  @ColorInt final int color) {
 ...
 */
@RequiresApi(Build.VERSION_CODES.LOLLIPOP)
public static void setNavBarColor(@NonNull final Window window,
                                  @ColorInt final int color) {
	window.addFlags(
		WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);

...
public final class AppUtils {
   ...
   * @param listener The status of application changed listener
   */
  public static void unregisterAppStatusChangedListener(
      @NonNull final Utils.OnAppStatusChangedListener listener) {
   ...
   * android:name="android.permission.REQUEST_INSTALL_PACKAGES" />}</p>
   *
   ...
   * Uninstall the app.
   * <p>Target APIs greater than 25 must hold
   * Must hold {@code <uses-permission
   * android:name="android.permission.REQUEST_DELETE_PACKAGES" />}</p>
   *
  ...
  public static void uninstallApp(final String packageName) {
    ...
    Utils.getApp().startActivity(
        UtilsBridge.getUninstallAppIntent(packageName));
  }
  ...
  public static boolean isAppForeground() {
    ...
    for (ActivityManager.RunningAppProcessInfo aInfo : info) {
      if ((aInfo.importance ==
          ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) && (aInfo.processName.equals(Utils.getApp().getPackageName()))) {
        return true;
      }
    }
    return false;
   ...
   * {@code <uses-permission
   * android:name="android.permission.PACKAGE_USAGE_STATS" />}</p>
   *
   * @param pkgName The name of the package.
   * @return {@code true}: yes<br>{@code false}: no
   */
  public static boolean isAppForeground(@NonNull final String pkgName) {
    return !UtilsBridge.isSpace(pkgName) &&
        pkgName.equals(UtilsBridge.getForegroundProcessName());
  }
  ...
  public static boolean isAppRunning(final String pkgName) {
    ...
    if (am != null) {
      ...
      List<ActivityManager.RunningServiceInfo> serviceInfo =
   ...
   */
  public static void relaunchApp() { relaunchApp(false); }
  ...
  public static void relaunchApp(final boolean isKillProcess) {
    Intent intent =
        UtilsBridge.getLaunchAppIntent(Utils.getApp().getPackageName());
    if (intent == null) {
      Log.e("AppUtils", "Didn't exist launcher activity.");
      return;
    }
  ...
  public static void launchAppDetailsSettings(final String pkgName) {
    if (UtilsBridge.isSpace(pkgName))
      return;
    Intent intent =
        UtilsBridge.getLaunchAppDetailsSettingsIntent(pkgName, true);
   ...
   */
  public static void launchAppDetailsSettings(final Activity activity,
  ...
  public static void launchAppDetailsSettings(final Activity activity,
    ...
    if (activity == null || UtilsBridge.isSpace(pkgName))
      return;
    Intent intent =
        UtilsBridge.getLaunchAppDetailsSettingsIntent(pkgName, false);
    if (!UtilsBridge.isIntentAvailable(intent))
      return;
  ...
  private static String getAppSignatureHash(final String packageName,
    ...
    return UtilsBridge
        .bytes2HexString(
            UtilsBridge.hashTemplate(signature[0].toByteArray(), algorithm))
  ...
  private static AppInfo getBean(final PackageManager pm,
    ...
    return new AppInfo(packageName, name, icon, packagePath, versionName,
                       versionCode, isSystem);
  }
  ...
  public static class AppInfo {
    ...
    public void setSystem(final boolean isSystem) { this.isSystem = isSystem; }

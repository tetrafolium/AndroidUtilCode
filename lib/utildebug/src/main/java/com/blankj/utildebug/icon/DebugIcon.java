...
public class DebugIcon extends RelativeLayout {
  ...
  public static DebugIcon getInstance() { return INSTANCE; }
  ...
  public DebugIcon() {
    ...
    TouchUtils.setOnTouchListener(this, new TouchUtils.OnTouchUtilsListener() {
      ...
      public boolean onMove(View view, int direction, int x, int y, int dx,
                            int dy, int totalX, int totalY, MotionEvent event) {
        view.setX(
            Math.min(Math.max(0, view.getX() + dx), rootViewWidth - viewWidth));
        view.setY(Math.min(Math.max(statusBarHeight, view.getY() + dy),
                           rootViewHeight - viewHeight));
        return true;
      }

      @Override
      public boolean onStop(View view, int direction, int x, int y, int totalX,
                            int totalY, int vx, int vy, MotionEvent event) {
      ...
      private void stick2HorizontalSide(View view) {
        view.animate()
            ...
            .translationX(view.getX() + viewWidth / 2f > rootViewWidth / 2f
                              ? rootViewWidth - viewWidth
                              : 0)
            .setDuration(100)
  ...
  private void wrapPosition() {
    post(new Runnable() {
      ...
      public void run() {
        ...
        setX(getX() + getWidth() / 2f > contentView.getWidth() / 2f

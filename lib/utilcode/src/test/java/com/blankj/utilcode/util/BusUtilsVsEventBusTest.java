...
public void setUp() throws Exception {
	// 这一步是在 AOP 的时候注入的，这里通过反射来注入 busUtilsFun
	// 事件，效果是一样的
	ReflectUtils getInstance =
		ReflectUtils.reflect(BusUtils.class).method("getInstance");
	getInstance.method("registerBus", "busUtilsFun",
...
public void compareRegister10000Times() {
	...
	compareWithEventBus(
		"Register 10000 times.", 10, 10000, new CompareCallback() {
			@Override
...
private void comparePostTemplate(String name, int subscribeNum,
                                 int postTimes) {
...
public void compareUnregister10000Times() {
	...
	compareWithEventBus("Unregister 10000 times.", 10, 1,
	                    new CompareCallback() {
			@Override
			public void runEventBus() {
			        for (BusUtilsVsEventBusTest test : tests) {
 ...
 */
private void compareWithEventBus(String name, int sampleSize, int times,

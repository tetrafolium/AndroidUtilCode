 ...
 * It will not restart Activity. u can put it in ur {@link
 * Activity#onCreate(Bundle)}.
 */
public static void applySystemLanguage() {
	if (isAppliedSystemLanguage())
		return;
	applyLanguage(Resources.getSystem().getConfiguration().locale, "", true,
	              false);
}
 ...
 * Apply the system language.
 *
 * @param activityClz The class of activity will be started after apply system
 *     language.
 */
public static void
applySystemLanguage(final Class<? extends Activity> activityClz) {
	applyLanguage(Resources.getSystem().getConfiguration().locale, activityClz,
	              true, true);
}
 ...
 * Apply the system language.
 *
 * @param activityClassName The full class name of activity will be started
 *     after apply system language.
 */
public static void applySystemLanguage(final String activityClassName) {
	applyLanguage(Resources.getSystem().getConfiguration().locale,
	              activityClassName, true, true);
}
 ...
 * @param activityClz The class of activity will be started after apply system
 *     language.
 *                    It will start the launcher activity if the class is
 * null.
 */
public static void
applyLanguage(@NonNull final Locale locale,
 ...
 *
 * @param locale            The language of locale.
 * @param activityClassName The class of activity will be started after apply
 *     system language.
 *                          It will start the launcher activity if the class
 * name is null.
 */
public static void applyLanguage(@NonNull final Locale locale,
...
private static void applyLanguage(@NonNull final Locale locale,
	...
	if (isNeedStartActivity) {
		...
		String realActivityClassName = TextUtils.isEmpty(activityClassName)
		                         ? UtilsBridge.getLauncherActivity()
		                         : activityClassName;
		intent.setComponent(
			new ComponentName(Utils.getApp(), realActivityClassName));
...
public static boolean isAppliedSystemLanguage() {
	return VALUE_FOLLOW_SYSTEM.equals(
		UtilsBridge.getSpUtils4Utils().getString(KEY_LOCALE));
}

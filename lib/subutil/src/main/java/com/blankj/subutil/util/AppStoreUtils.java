...
public final class AppStoreUtils {
  ...
  private static final String GOOGLE_PLAY_APP_STORE_PACKAGE_NAME =
      "com.android.vending";
  ...
  public static Intent getAppStoreIntent(boolean isIncludeGooglePlayStore) {
    return getAppStoreIntent(Utils.getApp().getPackageName(),
   ...
   */
  public static Intent getAppStoreIntent(final String packageName,
                                         boolean isIncludeGooglePlayStore) {
    ...
    List<ResolveInfo> resolveInfos =
        Utils.getApp().getPackageManager().queryIntentActivities(
  ...
  private static Intent getNormalAppStoreIntent() {
    Intent intent = new Intent();
    Uri uri =
        Uri.parse("market://details?id=" + Utils.getApp().getPackageName());
    intent.setData(uri);
  ...
  private static Intent getSamsungAppStoreIntent(final String packageName) {
    ...
    intent.setData(
        Uri.parse("http://www.samsungapps.com/appquery/appDetail.as?appId=" +
                  packageName));
    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    if (getAvailableIntentSize(intent) > 0) {
      return intent;

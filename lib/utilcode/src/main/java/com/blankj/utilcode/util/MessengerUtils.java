...
public class MessengerUtils {
  ...
  private static ConcurrentHashMap<String, MessageCallback> subscribers =
  ...
  public static void subscribe(@NonNull final String key,
                               @NonNull final MessageCallback callback) {
  ...
  static class Client {
    ...
    ServiceConnection mConn = new ServiceConnection() {
      ...
      public void onServiceConnected(ComponentName name, IBinder service) {
        ...
        Message msg =
            Message.obtain(mReceiveServeMsgHandler, WHAT_SUBSCRIBE, key, 0);
        msg.replyTo = mClient;
        try {
          mServer.send(msg);
    ...
    Client(String pkgName) { this.mPkgName = pkgName; }

    boolean bind() {
      if (TextUtils.isEmpty(mPkgName)) {
        Intent intent = new Intent(Utils.getApp(), ServerService.class);
        return Utils.getApp().bindService(intent, mConn,
      ...
      if (UtilsBridge.isAppInstalled(mPkgName)) {
        if (UtilsBridge.isAppRunning(mPkgName)) {
          ...
          Log.e("MessengerUtils",
                "bind: the app is not running -> " + mPkgName);
          return false;
        }
      } else {
        Log.e("MessengerUtils",
              "bind: the app is not installed -> " + mPkgName);
        return false;
      }
    }
    ...
    void unbind() {
      int key = UtilsBridge.getCurrentProcessName().hashCode();
      Message msg =
          Message.obtain(mReceiveServeMsgHandler, WHAT_UNSUBSCRIBE, key, 0);
      msg.replyTo = mClient;
      try {
        mServer.send(msg);
      } catch (RemoteException e) {
  ...
  public static class ServerService extends Service {
    ...
    private final ConcurrentHashMap<Integer, Messenger> mClientMap =
        new ConcurrentHashMap<>();
  ...
  public interface MessageCallback { void messageCall(Bundle data); }
}

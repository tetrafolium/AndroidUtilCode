...
public class FileItem extends BaseItem<FileItem> {
  ...
  private static final ArrayList<FileItem> EMPTY =
      CollectionUtils.newArrayList(new FileItem());
  ...
  public FileItem(File file, String name) { this(file, name, false); }
  ...
  public FileItem() { super(R.layout.du_item_empty); }
  ...
  public void bind(@NonNull ItemViewHolder holder, int position) {
      ...
      public void onClick(View v) {
        ...
        if (delete) {
          ...
          if (getAdapter().getItems().isEmpty()) {
            ...
            v.getRootView()
                .findViewById(R.id.fileExplorerSearchEt)
    ...
    if (mFile.isDirectory()) {
      ...
      fileInfoTv.setText(String.format(
          "%s  %s",
          StringUtils.getString(R.string.du_file_item_num,
                                CollectionUtils.size(mFile.list())),
          TimeUtils.millis2String(mFile.lastModified(), "yyyy.MM.dd")));
  ...
  public static List<FileItem> getFileItems(final FileItem parent) {
    if (parent == null)
      return getFileItems();
    List<File> files =
        FileUtils.listFilesInDir(parent.getFile(), new Comparator<File>() {
          @Override
    ...
    return (List<FileItem>)CollectionUtils.collect(
        files, new CollectionUtils.Transformer<File, FileItem>() {
          @Override
      ...
      for (int i = 0; i < mountedSDCardPath.size(); i++) {
        ...
        if (sdPath.exists()) {
          fileItems.add(new FileItem(
              sdPath, "sdcard" + i + "_" + sdPath.getName(), true));
        }
      }
    }
    return fileItems;
  ...
  public static List<FileItem> filterItems(List<FileItem> items,
                                           final String key) {
    return (List<FileItem>)CollectionUtils.select(
        items, new CollectionUtils.Predicate<FileItem>() {
          @Override

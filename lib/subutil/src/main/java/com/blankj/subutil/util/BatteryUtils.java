...
@IntDef({BatteryStatus.UNKNOWN, BatteryStatus.DISCHARGING,
         BatteryStatus.CHARGING, BatteryStatus.NOT_CHARGING,
         BatteryStatus.FULL})
 ...
 */
public static void registerBatteryStatusChangedListener(
	final OnBatteryStatusChangedListener listener) {
...
isRegistered(final OnBatteryStatusChangedListener listener) {
 ...
 * @param listener The status of battery changed listener.
 */
public static void unregisterBatteryStatusChangedListener(
	final OnBatteryStatusChangedListener listener) {
	...
	ThreadUtils.runOnUiThread(new Runnable() {
				...
				public void run() {
				        ...
				        if (preSize == 0 && mListeners.size() == 1) {
							...
							BatteryChangedReceiver.getInstance(), intentFilter);
					}
				}
			});
}
	...
	ThreadUtils.runOnUiThread(new Runnable() {
				...
				public void run() {
				        ...
				        if (preSize == 1 && mListeners.size() == 0) {
							...
							BatteryChangedReceiver.getInstance());
					}
				}
			});
}
...
private static final BatteryChangedReceiver INSTANCE =
...
private int level;
@BatteryStatus private int status;

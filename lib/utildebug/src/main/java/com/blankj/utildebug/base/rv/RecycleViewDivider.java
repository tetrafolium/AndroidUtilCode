...
public RecycleViewDivider(Context context, int orientation,
                          @DrawableRes int resId, boolean showFooterDivider) {
...
protected void drawVertical(Canvas canvas, RecyclerView parent) {
	...
	for (int i = 0; i < childCount; i++) {
		...
		final int bottom =
			mBounds.bottom + Math.round(ViewCompat.getTranslationY(child));
...
protected void drawHorizontal(Canvas canvas, RecyclerView parent) {
	...
	for (int i = 0; i < childCount; i++) {
		...
		final int right =
			mBounds.right + Math.round(ViewCompat.getTranslationX(child));
